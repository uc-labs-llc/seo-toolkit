...
def save_results_to_file(report_content, source_name):
    """Asks the user to save the report and handles file I/O."""
    save_choice = input("\nDo you want to save the audit results to a file? (y/n): ").strip().lower()
    
    if save_choice == 'y':
        # Clean the source name to create a default filename
        ...
        default_filename = f"{clean_name}_audit_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
